...
void SiendoArrojado::cambiarModo(){
	if (esUltimoModo()){
		if(this->cantModos>=2)
			setModoActual(1);
		else
...
void SiendoArrojado::cambiarModoInversamente(){
	if(this->cantModos >= 2){
...
CoordenadaLogica* SiendoArrojado::execute(CoordenadaLogica* coord_personaje){
...
bool SiendoArrojado::permiteAccion(accion_posible nuevaAccion){
	return false;
}

void SiendoArrojado::resetear(){
	Accion::resetear();
	dist_virtual = 0;
}
